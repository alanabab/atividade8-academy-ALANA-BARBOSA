...
Then('visualizarei o alerta {string}', function (text) {
  paginaLogin.getAlerta().should('be.visible');
  paginaLogin.getAlerta().invoke('text').should('equal', text);
});
...
Then('visualizarei os alertas de campos obrigatórios', function () {
  paginaLogin.CamposObrigatorios();
});

Then('visualizarei a mensagem de erro {string}', function (text) {
  paginaLogin.getJanela().should('be.visible');
  paginaLogin.getJanela().should('contain', 'Falha ao autenticar');
  paginaLogin.getJanela().should('contain', text);
});
